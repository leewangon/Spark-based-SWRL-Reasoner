...
//    val triples = sc.textFile("Data/0626.txt").map(parseTriple)
    ...
    while (deliberatedCount != 0) {
      var historyRDD:RDD[(List[String], String)] = sc.emptyRDD
...
//            history.collect().foreach(println)
            ...
            val hh: RDD[(List[String], String)] = history.map(h => (DeliberationToString(h._1), h._2.toString()))
//            hh.collect().foreach(println)

            historyRDD = historyRDD.union(hh)


            predicateRDD(predIndex) = predicateRDD(predIndex).union(inferredTriples).repartition(4) //.persist(StorageLevel.MEMORY_ONLY_SER)
      ...
      historyRDD.saveAsObjectFile("SWRL_History"+phaseCount)
      phaseCount += 1
    }
